...
const JoinGame = ({ onClose }) => {
  ...
  const handleWordsPerTurnChange = (event, newValue) =>
    setWordsPerTurn(newValue);

  const handleTeamAdd = (event) => {
    if (event.target.name === "red") setRedTeam([...redTeam, name]);
    if (event.target.name === "blue") setBlueTeam([...blueTeam, name]);
    setName("");
  };
  ...
  const handleSubmit = () => {
    if (redTeam.length >= 2 && blueTeam.length >= 2) navigate("/draw/" + code);
