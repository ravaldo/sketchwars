...
const DrawingTools = ({ setBrushColour, setBrushSize, clearCanvas }) => {
  ...
  const swatches = colours.map((colour) => {
    return (
      <div
        className={colour == "#000" ? "tool color selected" : "tool color"}
        key={colour}
        id={colour}
  ...
  return (
    <>
      <div className="toolbar">
        ...
        <div className="tool brush selected" onClick={handleOtherClick}>
          <div id="smallBrush"></div>
        </div>
        <div className="tool brush" onClick={handleOtherClick}>
          <div id="mediumBrush"></div>
        </div>
        <div className="tool brush" onClick={handleOtherClick}>
          <div id="largeBrush"></div>
        </div>
        <div className="tool blank"></div>
        <div className="tool brush" onClick={handleOtherClick}>
